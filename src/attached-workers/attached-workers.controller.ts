...
export class AttachedWorkersController {
  constructor(private readonly attachedWorkersService: AttachedWorkersService) {}

  @Post()
  @ApiOperation({ summary: 'Attach a worker to an OrderWorker' })
  @ApiResponse({ status: 201, description: 'AttachedWorker created successfully' })
  ...
  @Get()
  @ApiOperation({ summary: 'Get list of attached workers with optional filters' })
  @ApiQuery({ name: 'skip', required: false })
  @ApiQuery({ name: 'take', required: false })
  @ApiQuery({ name: 'cursor', required: false })
  @ApiQuery({ name: 'orderBy', required: false, example: 'createdAt:desc' })
  @ApiResponse({ status: 200, description: 'List of attached workers returned' })
  findAll(
  ...
  @ApiParam({ name: 'id' })
  @ApiResponse({ status: 200, description: 'Attached worker updated successfully' })
  ...
  @ApiParam({ name: 'id' })
  @ApiResponse({ status: 200, description: 'Attached worker deleted successfully' })
